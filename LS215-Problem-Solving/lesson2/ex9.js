...
ALGORITHM:
  ...
  1. Find all the positive words in the text.
  2. Find all the negative words in the text.
...
function matchWords (text, wordsToMatch) {
  ...
  wordsToMatch.forEach(word => {
    ...
    let wordMatches = cleanedText.match(regex);
    if (wordMatches) matches.push(...wordMatches);
  });
  return matches;
}
...
function logSentimentReport(
  ...
  console.log(`Positive sentiments: ${negativeWordsInTheText.join(', ')}\n`);
...
// console.log(removeSpecialChars(textExcerpt));
// matchWords(textExcerpt,['fortune', 'dream', 'love', 'respect', 'patience', 'devout', 'noble', 'resolution']);
// matchWords(textExcerpt, ['die', 'heartache', 'death', 'despise', 'scorn', 'weary', 'trouble', 'oppress']);
